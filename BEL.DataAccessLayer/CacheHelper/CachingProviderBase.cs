namespace BEL.DataAccessLayer
{
    ...
    public abstract class CachingProviderBase : IDisposable
    {
        ...
        protected virtual void AddItem(string key, object value)
        {
            lock (Padlock)
            {
                if (CacheMode && !CachingNotRequiredFor.Contains(key))
                {
                    CacheItemPolicy policy = new CacheItemPolicy();
                    if (CacheInterval.ContainsKey(key))
                    {
                        policy.AbsoluteExpiration = DateTimeOffset.Now.AddHours(Convert.ToDouble(CacheInterval[key]));
                    }
                    else
                    {
        ...
        {
            File.Delete(string.Format("{0}\\CachingProvider_Errors.txt", logPath));
        }
        ...
        {
            using (TextWriter tw = File.AppendText(string.Format("{0}\\CachingProvider_Errors.txt", logPath)))
            {
                tw.WriteLine(text);
                tw.Close();
            }
