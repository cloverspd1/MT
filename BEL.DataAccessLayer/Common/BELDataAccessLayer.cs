namespace BEL.DataAccessLayer
{
    ...
    using BEL.DataAccessLayer.Common;
    ...
    public class BELDataAccessLayer
    {
        ...
        public static string GenerateReferenceNo(string appName, string itemId, DateTime requestDate)
        {
            ...
            switch (appName)
            {
                case ApplicationNameConstants.MATERIALTRACKINGAPP:
                    appName = "MATERIALTRACKING";
                    break;
                case FormNameConstants.INWARDREQUESTFORM:
                    appName = "INREQ";
                    break;
                case FormNameConstants.OUTWARDASSMBLREQUESTFORM:
                    appName = "OUTREQ";
                    break;
            }
        ...
        public static string GetAliasFromPersonField(ClientContext context, Web web, FieldUserValue[] users)
        {
            ...
            if (users != null && users.Length > 0 && web != null && context != null)
            {
                foreach (FieldUserValue user in users)
                {
                    Logger.Info("Helper.GetAliasFromPersonField", user.LookupId);
                    string emailId;
                    ...
                    Logger.Info("Helper.GetNameFromPersonField", emailId);
                    if (alise != null)
                    {
                        personsName = personsName.Trim(',') + ", " + alise;
                    }
                }
            }
        ...
        /// <returns>FielduserValue Object</returns>
        public static string GetAliaseFromUserId(ClientContext context, Web web, string userIDs)
        {
            MasterDataHelper masterHelper = new MasterDataHelper();
            string userAlises = string.Empty;
            var ides = userIDs.Split(',');
            var lstFieldUserValues = new List<FieldUserValue>();
            foreach (var id in ides)
            {
                var varUser = EnsureUser(context, web, Convert.ToString(id));
                var emailId = varUser.Email;
                var alise = masterHelper.GetEmployeeAliasByEmail(context, web, emailId);
                userAlises += alise + ",";
            }

            if (!string.IsNullOrEmpty(userAlises)) userAlises = userAlises.Trim(',');
            return userAlises;
        }
        ...
        public static string GetNameUsingUserID(ClientContext context, Web web, string ids)
        {
            ...
            if (context != null && web != null && !string.IsNullOrEmpty(ids))
            {
                ...
                foreach (string id in userIds)
                {
                    ...
                    UserDetails detail = userInfoList.FirstOrDefault(p => p.UserId == id.Trim());
            ...
            if (context != null && web != null && param != null && item != null)
            {
                ...
                if (!string.IsNullOrEmpty(approverMatrixListName))
                {
                    ...
                    if (!approversDataFromList.Any(p => p.IsOptional == false && !string.IsNullOrEmpty(p.Approver) && p.Status != ApproverStatus.APPROVED && Convert.ToInt32(p.Levels) == currLevel)
                        && actionPerformed != ButtonActionStatus.SendBack && actionPerformed != ButtonActionStatus.Forward && actionPerformed != ButtonActionStatus.SendForward && actionPerformed != ButtonActionStatus.BackToCreator)
                    {
                    ...
                    if ((actionPerformed == ButtonActionStatus.SendBack || actionPerformed == ButtonActionStatus.BackToCreator) && param.ContainsKey(Parameter.SENDTOLEVEL) && param[Parameter.SENDTOLEVEL] != null && !string.IsNullOrEmpty(param[Parameter.SENDTOLEVEL]))
                    {
                ...
                switch (actionPerformed)
                {
                    ...
                    case ButtonActionStatus.BackToCreator:
                        ...
                        formFieldValues.Add("LastActionBy", GetFieldUserValueFromPerson(context, web, currentUserID));
                        formFieldValues.Add("LastActionByRole", currentUserRole);
                        //Wriiten by Priya on 21 March for change creator to Tester
                        //formFieldValues.Add("PendingWith", nextApproverRole);
                        formFieldValues.Add("PendingWith", "Tester");
                        if (!string.IsNullOrEmpty(nextApprover))
                        {
                            FieldUserValue[] nextApproverFieldValue = GetFieldUserValueFromPerson(context, web, nextApprover.Trim(',').Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries));
                            formFieldValues.Add("NextApprover", nextApproverFieldValue);
                ...
                {
                    if (formFieldValues["Status"] != "Completed")
                    {
                        FieldUserValue[] userValues = formFieldValues["NextApprover"];
                        string usersAliasFromPersonField = GetAliasFromPersonField(context, web, userValues);
                        formFieldValues.Add("NextApproverAlias", usersAliasFromPersonField);
                    }
                    else if (formFieldValues["Status"] == "Completed")
                    {
                        formFieldValues.Add("NextApproverAlias", "");
                    }
                }
        ...
        private void SendMail(ButtonActionStatus actionPerformed, ClientContext context, Web web, string currentUserID, int itemId, List<ApplicationStatus> approversDataFromList, string listname, int nextLevel, int currLevel, Dictionary<string, string> paraml, Dictionary<string, string> mailCustomValues = null, List<FileDetails> emailAttachments = null)
        {
            ...
            switch (actionPerformed)
            {
                ...
                case ButtonActionStatus.NextApproval:
                    ...
                    {
                        from = currentUserID;
                        var varUser = EnsureUser(context, web, Convert.ToString(currentUserID));
                        var emailId = varUser.Email;
                        var alise = masterHelper.GetEmployeeAliasByEmail(context, web, emailId);
                        if (alise != null)
                        {
                            if (mailCustomValues != null) mailCustomValues[Parameter.CURRENTUSERALISE] = alise;
                        }
                ...
                case ButtonActionStatus.Hold:
                    ...
                    {
                        from = currentUserID;
                        if (mailCustomValues != null) mailCustomValues[Parameter.CURRENTUSERALISE] = GetAliaseFromUserId(context, web, currentUserID);
                        ...
                        email = eHelper.GetEmailBody(context, web, EmailTemplateName.HOLDMAIL, itemdetail, mailCustomValues, role, paraml[Parameter.APPLICATIONNAME], paraml[Parameter.FROMNAME]);
                    }
                    break;
                ...
                case ButtonActionStatus.Resume:
                    if (approversDataFromList != null && approversDataFromList.Count != 0)
                    {
                        ...
                        email = eHelper.GetEmailBody(context, web, EmailTemplateName.RESUMEMAIL, itemdetail, mailCustomValues, role, paraml[Parameter.APPLICATIONNAME], paraml[Parameter.FROMNAME]);
                    }
                    break;
                case ButtonActionStatus.SendBack:
                    if (approversDataFromList != null && approversDataFromList.Count != 0)
                ...
                case ButtonActionStatus.BackToCreator:
                    if (approversDataFromList != null && approversDataFromList.Count != 0)
                    {
                        ...
                        cc = (cc.Trim(',') + "," + approversDataFromList.FirstOrDefault(p => p.Role == UserRoles.CREATOR).Approver).Trim(',');
                        role = approversDataFromList.FirstOrDefault(p => Convert.ToInt32(p.Levels) == currLevel).Role;
                        tmplName = EmailTemplateName.SENDBACKTOCREATORMAIL;
                        email = eHelper.GetEmailBody(context, web, tmplName, itemdetail, mailCustomValues, role, paraml[Parameter.APPLICATIONNAME], paraml[Parameter.FROMNAME]);
                    }
                    //// creato to all viewer
                    break;
                case ButtonActionStatus.Cancel:
        ...
        public IForm GetFormData(ClientContext context, Web web, string applicationName, string formName, int itemId, string userID, IForm form, string role, Dictionary<string, bool> activeAndHiddenSections, Dictionary<string, string> otherParamDictionary)
        {
            ...
            if (context != null && web != null && !string.IsNullOrEmpty(applicationName) && !string.IsNullOrEmpty(formName) && !string.IsNullOrEmpty(userID) && form != null && !string.IsNullOrEmpty(role) && activeAndHiddenSections != null)
            {
                ...
                List<EmployeeDetails> employeeDetailsesFromList = null;
                if (itemId > 0)
                {
                        ...
                        else
                        {
                            ...
                            if (form.SectionsList[i].ListDetails[0].ListItemObject != null)
                            {
                                ...
                                if (form.SectionsList[i].ListDetails[0].ListItemObject.FieldValues.ContainsKey("Status") && form.SectionsList[i].ListDetails[0].ListItemObject.FieldValues.ContainsKey("NextApprover"))
                                {
                                    ...
                                    {
                                        approvers = GetAliasFromPersonField(context, web, form.SectionsList[i].ListDetails[0].ListItemObject["NextApprover"] as FieldUserValue[]);
                                    }
                        ...
                        if (isListColumn && isCurrentListCoumn)
                        {
                            ...
                            else if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson && !property.GetCustomAttribute<IsPersonAttribute>().ReturnName)
                            {
                                ...
                                if (users != null)
                                {
                                    ...
                                    {
                                        personEmails = GetAliasFromPersonField(context, web, users);
                                    }
                                    else
                                    {
