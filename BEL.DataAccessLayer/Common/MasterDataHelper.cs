    ...
    public class MasterDataHelper : BELDataAccessLayer
    {
        ...
        public List<IMaster> GetMasterData(ClientContext context, Web web, List<IMaster> masters)
        {
            ...
            if (context != null && web != null && masters != null && masters.Count != 0)
            {
                ...
                foreach (IMaster mstr in masters)
                {
                    ...
                    {
                        master = GetMasterDataOfUsingREST(context, web, mstr);
                    }
                    else
                    {
                        master = GetMasterDataOf(context, web, mstr);
                    }
        ...
        private IMaster GetMasterDataOf(ClientContext context, Web web, IMaster master)
        {
            ...
            if (objMaster == null)
            {
                ...
                string viewFields = GetViewFields(properties);
                if (scope.Equals(ListScope.GLOBAL))
                {
                    ...
                    context = CreateClientContext(siteURL);
                    web = CreateWeb(context);
                }
        ...
        private IMaster GetMasterDataOfUsingREST(ClientContext context, Web web, IMaster master)
        {
            ...
            if (objMaster == null)
            {
                ...
                if (scope.Equals(ListScope.GLOBAL))
                {
                    siteURL = GetSiteURL(SiteURLs.ROOTSITEURL); //this.GetConfigVariable(SiteURLs.ROOTSITEURL);
                }
                ...
                foreach (PropertyInfo property in itemProperties)
                {
                    if (property.GetCustomAttribute<FieldColumnNameAttribute>() != null && !string.IsNullOrEmpty(property.GetCustomAttribute<FieldColumnNameAttribute>().FieldsInformation))
                    {
                        if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson && !property.GetCustomAttribute<IsPersonAttribute>().ReturnName)
                        {
                            ...
                            {
                                property.SetValue(listItem, GetEmailsFromPersonField(context, web, (FieldUserValue[])item[property.GetCustomAttribute<FieldColumnNameAttribute>().FieldsInformation]));
                            }
                            else
                            {
                                property.SetValue(listItem, GetEmailsFromPersonField(context, web, (FieldUserValue)item[property.GetCustomAttribute<FieldColumnNameAttribute>().FieldsInformation]));
                        ...
                        else if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson && property.GetCustomAttribute<IsPersonAttribute>().ReturnName)
                        {
                            if (item[property.GetCustomAttribute<FieldColumnNameAttribute>().FieldsInformation] is FieldUserValue[])
                            {
                                property.SetValue(listItem, GetAliasFromPersonField(context, web, (FieldUserValue[])item[property.GetCustomAttribute<FieldColumnNameAttribute>().FieldsInformation]));
                            }
                            else
                            {
                                property.SetValue(listItem, GetNameFromPersonField(context, web, (FieldUserValue)item[property.GetCustomAttribute<FieldColumnNameAttribute>().FieldsInformation]));
                            }
                        }
                        else if (property.GetCustomAttribute<FieldColumnNameAttribute>().IsLookup)
                        {
        ...
        private List<EmployeeDetails> GetAllEmployeeDetails(ClientContext context, Web web)
        {
            ...
            string siteUrl = GetSiteURL(SiteURLs.MTSITEURL); //this.GetConfigVariable(SiteURLs.ROOTSITEURL);
            List slaList = web.Lists.GetByTitle(ListNames.EmployeeDetailMasterList);
        ...
        public List<EmployeeDetails> GetEmployeeDetailsList(ClientContext context, Web web)
        {
            var employeeDetailsesList =
                GlobalCachingProvider.Instance.GetItem(ListNames.EmployeeDetailMasterList, false) as
                    List<EmployeeDetails>;
            if (employeeDetailsesList == null)
            {
                employeeDetailsesList = GetAllEmployeeDetails(context, web);
                GlobalCachingProvider.Instance.AddItem(ListNames.EmployeeDetailMasterList, employeeDetailsesList);
            }
        ...
        public EmployeeDetails GetEmployeeByEmail(ClientContext context, Web web, string email)
        {
            var emplDetailList = GlobalCachingProvider.Instance.GetItem(ListNames.EmployeeDetailMasterList, false) as List<EmployeeDetails> ??
                                                   GetEmployeeDetailsList(context, web);
            if (email != null)
                return emplDetailList.Find(details => (details != null && !string.IsNullOrWhiteSpace(details.Email)) ? details.Email.ToLower().Equals(email.ToLower()) : false);
            else
                return null;
        ...
        {
            var employee = GetEmployeeByEmail(context, web, email);
