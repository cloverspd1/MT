...
namespace BEL.DataAccessLayer
{
    ...
    public class EmailHelper
    {
        ...
        /// </returns>
        public Dictionary<string, string> GetEmailBody(ClientContext appContext, Web appWeb, string templateName, List<ListItemDetail> listItemDetails, Dictionary<string, string> customValues, string role, string applicationName = ApplicationNameConstants.MATERIALTRACKINGAPP, string formName = FormNameConstants.INWARDREQUESTFORM)
        {
                ...
                using (ClientContext context = BELDataAccessLayer.Instance.CreateClientContext(siteURL))
                {
                    ...
                    if (items.Count > 0)
                    {
                        ...
                        if (listItemDetails != null || customValues != null)
                        {
                            ...
                            emailTemplate = CreateEmailBody(appContext, appWeb, emailTemplate, listItemDetails, customValues);
                        }
                    }
                }
            }
                    ...
                    foreach (KeyValuePair<string, string> strEmail in emailBody)
                    {
                        ...
                        foreach (Match match in mc)
                        {
                            ...
                            try
                            {
                                ...
                                preparedEmail = preparedEmail.Replace("[[Custom:" + fieldName + "]]", GetFieldValueString(customValues[fieldName]));
                            }
                            catch (Exception ex)
                            {
                        ...
                        foreach (KeyValuePair<string, string> strEmail in emailBodyWithCustomData)
                        {
                            ...
                            foreach (Match match in mc)
                            {
                                ...
                                try
                                {
                                    ...
                                    preparedEmail = preparedEmail.Replace("[[" + listName + ":" + fieldName + "]]", GetFieldValueString(item[fieldName]));
                                }
                                catch (Exception ex)
                                {
                                    Logger.Error("Error While Replace parameter, Error Message: " + ex.Message + " Stack trace :  " + ex.StackTrace);
