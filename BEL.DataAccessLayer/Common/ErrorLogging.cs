namespace BEL.DataAccessLayer
{
    ...
    public class ErrorLogging
    {
        ...
        public static void LogError(string className, string methodName, string msg, Exception ex)
        {
            ...
            {
                Exception newExe = ex;
                string stackTrace = "[Message: " + newExe.Message + "]" + Environment.NewLine + newExe.StackTrace;
                while (newExe.InnerException != null)
                {
                    stackTrace += "[Message: " + newExe.Message + "]" + Environment.NewLine + ex.InnerException.StackTrace;
