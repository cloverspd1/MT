namespace System.Web.Mvc.Html
{
    ...
    public static class Helper
    {
        ...
        public static List<EmployeeDetailsMasterListItem> splitUser(List<EmployeeDetailsMasterListItem> items)
        {
            List<EmployeeDetailsMasterListItem> tempDataList = new List<EmployeeDetailsMasterListItem>();
            if (items != null)
            {
                foreach (EmployeeDetailsMasterListItem item in items)
                {
                    if (item.UserSelection == true && !string.IsNullOrEmpty(item.UserID) && item.UserID.Contains(','))
                    {
                        ...
                        {
                            EmployeeDetailsMasterListItem temprow = new EmployeeDetailsMasterListItem();
                            temprow.Title = item.Title;
                            temprow.UserName = Convert.ToString(strUserName[i]).Trim();
                            temprow.UserID = strUserID[i];
                            temprow.AliasNames = item.AliasNames;
